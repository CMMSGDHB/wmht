...
api.get("/usermessage", function (req, res) {
    ...
    //获取单个shoplists表数据
    db.query("select * from user_info where realname='" + name + "'", function (error, results, fields) {
        if (results.length == 0) {
            res.setHeader('content-type', 'text/html;charset=utf-8');
...
api.post("/insertuserinfo", (req, res) => {
    ...
    let sql = "insert into user_info(realname,phone,email)values('" + username + "','" + phone + "','" + email + "')";
    console.log(sql)
    db.query(sql, (error, results, fields) => {
        // console.log(results);
        if (results.affectedRows > 0) {
...
api.post("/updateuserpic", (req, res) => {
    ...
    let sql = "update user_info set pic='" + headerpic + "' where realname='" + realname + "'";
    // console.log(sql)
    db.query(sql, (error, results, fields) => {
        // console.log(results);
        if (results.affectedRows > 0) {
